...
class UnetSelect:
    CATEGORY = 'SaveImageTools/utils'
    RETURN_TYPES = (folder_paths.get_filename_list("unet"),)
...
class SaveFluxImageWithMetadata:
    def __init__(self):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "modelname": (folder_paths.get_filename_list("unet"),),
    ...
    def save_files(self, images, seed_value, steps, cfg, sampler_name, scheduler, positive, negative, modelname, quality_jpeg_or_webp,
        ...
        ckpt_path = folder_paths.get_full_path("unet", modelname)      
# 2025-02-04 Enhancement - Look checkpoint from checkpoints folder for non Flux checkpoints
...
#
    ...
    def save_images(self, images, output_path, filename_prefix, comment, extension, quality_jpeg_or_webp, lossless_webp, prompt=None, extra_pnginfo=None) -> list[str]:
        ...
        paths = list()
# 2024-10-10 Patch
        filename_prefix_original = filename_prefix
#
        for image in images:
            i = 255. * image.cpu().numpy()
            img = Image.fromarray(np.clip(i, 0, 255).astype(np.uint8))
# 2024-10-10 Patch
            filename_prefix = filename_prefix_original
#
            if images.size()[0] > 1:
...
NODE_CLASS_MAPPINGS = {
    "Unet Select": UnetSelect,
    "Save Flux Image with Metadata": SaveFluxImageWithMetadata,
    "Sampler Select": SamplerSelect,
    "Scheduler Select": SchedulerSelect,
    "Seed Gen": SeedGen,
